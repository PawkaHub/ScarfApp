'use strict';
// This object updates all of the boxes from the constraint solver. It also tests all of the motion constraints and identifies which manipulator caused a motion constraint to be violated.
...
MotionContext.prototype.solver = function() {
    return this._solver;
}
MotionContext.prototype.addBox = function(box) {
    this._boxes.push(box);
}
MotionContext.prototype.boxes = function() {
    return this._boxes;
}
...
MotionContext.prototype.addManipulator = function(manipulator) {
    //console.log('addManipulator', manipulator, this);
    this._manipulators.push(manipulator);
    manipulator._setMotionContext(this);
    this.update(); // XXX: Remove -- constructing a Manipulator used to do this, moved it here but it should go. We use this here to forcibly update the box's positions before the user has interacted with them
    return manipulator;
}
MotionContext.prototype.update = function() {
        // Prevent re-entrancy which can happen when a motion constraint violation causes an animation to be created which propagates another update.
        if (this._updating) return;
        //console.log('motionContext update', this);
        this._updating = true;
        this._resolveMotionConstraints();
        for (var i = 0; i < this._boxes.length; i++) {
            //console.log('updating!', this._boxes[i].x.value);
...
MotionContext.prototype._resolveMotionConstraints = function() {
    ...
    for (var i = 0; i < this._manipulators.length; i++) {
        ...
        allViolations[manipulator.name()] = {
